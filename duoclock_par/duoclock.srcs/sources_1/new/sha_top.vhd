...
architecture PHY of sha_top is
	...
	signal clkfbout, Clk, Clk_l, ClkTX_l, ClkTX, ClkS, ClkS_l : std_logic;
...
--	ID <= ID_in when (ID_in /= "111") else IDIO_fixed;
	...
	SHAFCE: entity WORK.SHA_FullCore generic map (LOC_Y => 0, NONCE_START => FC_NONCE_START)
		port map (Clk => Clk, ClkS => ClkS, ProgC => ProgC, ProgD => ProgD, Match => MatchCoreEven);
	SHAFCO: entity WORK.SHA_FullCore generic map (LOC_Y => 1, NONCE_START => FC_NONCE_START xor x"00000001")
		port map (Clk => Clk, ClkS => ClkS, ProgC => ProgC, ProgD => ProgD, Match => MatchCoreOdd);
	...
	SHACONFIG: entity WORK.SHA_FC_Config generic map (NONCE_START => FC_NONCE_START)
		port map (ClkS => ClkS, ClkTX => ClkTX, MC_A => MC_A_slow, MC_D => MC_D_slow, 
	...
	DCMCI: DCM_CLKGEN generic map (
			CLKFXDV_DIVIDE => 2,
			CLKFX_DIVIDE => 1,
			CLKFX_MD_MAX => 17.0, -- Maximum clock is 380 Mhz!
			CLKFX_MULTIPLY => 10, -- Default clock is 200 Mhz!
			CLKIN_PERIOD => 50.0,
			SPREAD_SPECTRUM => "NONE",
			STARTUP_WAIT => FALSE
		) port map (
			CLKFX => Clk_l,           -- Main clock
			CLKFX180 => open,
			CLKFXDV => open, -- ClkS_l,        -- Clock divided by 2
	...
	-- This is _additional_ simply to shift phase!
	DCMSPG: DCM_SP generic map (
		CLKDV_DIVIDE => 2.0, CLKFX_DIVIDE => 1,	CLKFX_MULTIPLY => 4, CLKIN_DIVIDE_BY_2 => TRUE,
		CLKIN_PERIOD => 2.9, CLKOUT_PHASE_SHIFT => "NONE",
		CLK_FEEDBACK => "1X", DESKEW_ADJUST => "SYSTEM_SYNCHRONOUS", PHASE_SHIFT => 0, STARTUP_WAIT => FALSE
	) port map (
		CLK0 => ClkS_l,
		CLK180 => open, CLK270 => open, CLK2X => open, CLK2X180 => open, CLK90 => open, CLKDV => open, CLKFX => open, CLKFX180 => open,
		LOCKED => open, PSDONE => open, STATUS => open, DSSEN => '0', 
		CLKFB => ClkS, CLKIN => Clk, PSCLK => '0', PSEN => '0', PSINCDEC => '0', RST => not DCM_Locked
	);
	...
	BUFGCLKS:   BUFG port map (O => ClkS, I => ClkS_l);
