...
architecture PHY of sha_top is
	...
	signal ProgC, ProgD, MatchCore : std_logic;

	signal Match_has, ScanDone_in : std_logic;
	signal m_shf, MC_D_slow, Match_data : std_logic_vector(31 downto 0);
...
--	ID <= ID_in when (ID_in /= "111") else IDIO_fixed;
	...
	SHAFCE: entity WORK.SHA_FullCore generic map (NONCE_START => FC_NONCE_START)
		port map (Clk => Clk, ProgC => ProgC, ProgD => ProgD, Match => m_shf(0));

	process (Clk) begin if rising_edge(Clk) then
		m_shf(31 downto 1) <= m_shf(30 downto 0);
	end if; end process;
	MatchCore <= m_shf(31);
	...
	SHACONFIG: entity WORK.SHA_FC_Config generic map (NONCE_START => FC_NONCE_START)
		port map (Clk => Clk, ClkTX => ClkTX, MC_A => MC_A_slow, MC_D => MC_D_slow, 
		          ProgC => ProgC, ProgD => ProgD, MatchCore => MatchCore,
	...
	DCMCI: DCM_CLKGEN generic map (
			CLKFXDV_DIVIDE => 2,
			CLKFX_DIVIDE => 1,
			CLKFX_MD_MAX => 20.0, -- Maximum clock is 400 Mhz!
